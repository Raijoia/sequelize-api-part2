...
class PessoaController extends Controller {
  ...
  async pegaTodosDesativados(req, res) {
    try {
      const listaTodasAsPessoas =
        await pessoaServices.pegaPessoasEscopoDesativados();
      return res.status(200).json(listaTodasAsPessoas);
    } catch (erro) {
      return res.status(500).json({ erro: erro.message });
    }
  ...
  async cancelaRegistroEstudante(req, res) {
    ...
    try {
      ...
      return res.status(200).json({ mensagem: `matriculas refereentes ao estudante ${estudante_id} canceladas`});
    } catch (erro) {
      return res.status(500).json({ erro: erro.message });
    }
  }
